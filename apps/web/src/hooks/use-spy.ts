...
function useSpy({
  ...
  const formatRefs = useCallback(
    (elementRefs: ElementRefList): SpiedElement[] =>
  ...
  const minMaxPair = useCallback((n: number[]): MinMaxPair => {
    return [Math.min(...n), Math.max(...n)];
  }, []);

  const calculateSpiedZones = useCallback(
    (spiedElements: SpiedElement[]): Map<MinMaxPair, KeyedElements[]> =>
    ...
    containerElem.addEventListener(
      ...
      throttle(() => {
        ...
        const currZoneIndex = availableSpyZones.findIndex(
          ([[minY, maxY]]) =>
            containerElem.scrollTop + offset > minY &&
            containerElem.scrollTop + offset < maxY,
        );
